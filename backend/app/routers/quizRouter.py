...
def get_all_quizzes(db: Session = Depends(get_db), current_user = Depends(get_current_user), params: SortSchema = Depends() ):
    return quizServices.get_all_quizzes(db, current_user, params)

@router.get("/{quiz_id}", response_model=QuizResponse)
def get_quiz_by_id(quiz_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    quiz = quizServices.get_quiz_by_id(db, quiz_id, current_user)
    if not quiz:
