from fastapi import APIRouter, Depends
from sqlalchemy.orm import Session
from ..core.dbConfig import get_db
from .. import schemas
from ..services import moduleServices as services

router = APIRouter(
    prefix="/modules",
    tags=["modules"]
)

@router.post("/", response_model=schemas.ModuleResponse)
def create_module(Module: schemas.ModuleBase, db: Session = Depends(get_db)):
    return services.create_module(Module, db)

@router.get("/", response_model=list[schemas.ModuleResponse])
def get_modules(db: Session = Depends(get_db)):
    return services.get_all_modules(db)

@router.get("/{module_id}", response_model=schemas.ModuleResponse)
def get_module(module_id: int, db: Session = Depends(get_db)):
    return services.get_module_by_id(module_id, db)

@router.put("/{module_id}", response_model=schemas.ModuleResponse)
def update_module(module_id: int, updated_module: schemas.ModuleUpdate, db: Session = Depends(get_db)):
    return services.update_module(module_id, updated_module, db)

@router.delete("/{module_id}")
def delete_module(module_id: int, db: Session = Depends(get_db)):
    return services.delete_module(module_id, db)