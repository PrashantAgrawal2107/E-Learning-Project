...
@router.put("/{instructor_id}", response_model=instructorSchema.InstructorResponse)
def update_instructor(instructor_id: int, updated_instructor: instructorSchema.InstructorUpdate, db: Session = Depends(get_db)):
    return instructorServices.update_instructor(instructor_id, updated_instructor, db)
